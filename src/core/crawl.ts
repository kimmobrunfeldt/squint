...
function normalizeUrl(url: string, opts: Pick<Config, 'includeHash' | 'trailingSlashMode'>): string {
...
type CrawlFilterFunctions = {
  [name in CrawlFilter]: (opts: CrawlFilterOptions, currentUrl: string, href: string) => boolean;
}
const crawlFilterFunctions: CrawlFilterFunctions = {
  siteInternal: (opts, currentUrl, href) => {
    ...
    const isInternal = urlNewParts.host.toLowerCase() === hrefUrlParts.host.toLowerCase();
    return isInternal;
  }
}

export function createShouldVisit(crawlFilters: CrawlFilter[], opts: CrawlFilterOptions): NonNullable<CrawlInputs["shouldVisit"]> {
  return function shouldVisit(currentUrl, href) {
    const filterResults = crawlFilters.map(name => crawlFilterFunctions[name](opts, currentUrl, href));
...
export async function crawlPaths(inputs: CrawlInputs, memory: CrawlInternalMemory = { visited: new Set<string>(), depth: 0 }): Promise<Array<string>> {
  const {
    pagePool,
    urlsToVisit,
    shouldVisit = () => true,
  ...
  urlsToVisit.forEach((url) => {
    const visitTask = async () => {
      const hrefs = await pagePool.use(async (page) => {
        ...
        return page.$$eval('a', (as) => (as as HTMLAnchorElement[]).map(a => a.href));
      });
